...
public class ProfesseurRESTController {
	...
	@RequestMapping(value="/profsByName/{nom}",method = RequestMethod.GET)
	public List<Professeur> findByNomProduitContains(@PathVariable("nom") String nom) {
	return professeurService.findByNomProfContains(nom);

	}


	
	@RequestMapping(value="/profscou/{idCours}",method = RequestMethod.GET)
	public List<Professeur> getProduitsByIdCours(@PathVariable("idCours") Long id) {
	return professeurService.findByCoursIdCours(id);
	}
